// src/app/products/[slug]/ProductPageClient.jsx
"use client";
...
const ProductPageClient = ({ product }) => {
	...
	let extras = 0;
	if (selection === "extended-eula") extras += 10;
	if (supportSelection === "enhanced-support") extras += 20;
							...
							{product.discountedPrice ? (
								<div className="flex gap-2 justify-center items-center align-middle">
									<span className="original-price">
										€{((product.price + extras) * 1).toFixed(2)}
									</span>
									<span className="discounted-price">
										€{((product.discountedPrice + extras) * 1).toFixed(2)}
									</span>
								</div>
							) : (
								<span className="price">
									€{(product.price + extras).toFixed(2)}
								</span>
							)}
						</div>
